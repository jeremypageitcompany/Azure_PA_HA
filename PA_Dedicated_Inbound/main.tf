...
resource "azurerm_public_ip" "pip-untrust-001" {

  name                = "pip_untrust_palovm3-${var.project}-${var.env}-${var.location}"
  location            = var.location
  resource_group_name = azurerm_resource_group.rg-001.name
  allocation_method   = "Static"
  tags                = var.resource_tags
...
resource "azurerm_public_ip" "pip-untrust-002" {

  name                = "pip_untrust_palovm4-${var.project}-${var.env}-${var.location}"
  location            = var.location
  resource_group_name = azurerm_resource_group.rg-001.name
  allocation_method   = "Static"
  tags                = var.resource_tags
