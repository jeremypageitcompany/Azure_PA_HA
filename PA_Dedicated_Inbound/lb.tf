...
resource "azurerm_lb_backend_address_pool_address" "lb-public-backend-address-001" {
  ...
  name                    = "lb_public_backend_address-palovm1-${var.project}-${var.env}-${var.location}"
  ...
  ip_address              = var.palovm.palovm1.untrust_ip
  depends_on = [
    azurerm_resource_group.rg-001
  ]
}

resource "azurerm_lb_backend_address_pool_address" "lb-public-backend-address-002" {

  name                    = "lb_public_backend_address-palovm2-${var.project}-${var.env}-${var.location}"
  backend_address_pool_id = azurerm_lb_backend_address_pool.lb-public-backend-001.id
  virtual_network_id      = azurerm_virtual_network.vnet-001.id
  ip_address              = var.palovm.palovm2.untrust_ip
  depends_on = [
    azurerm_resource_group.rg-001
  ]
}
...
resource "azurerm_lb_backend_address_pool_address" "lb-internal-backend-address-001" {
  ...
  name                    = "lb_internal_backend_address-palovm3-${var.project}-${var.env}-${var.location}"
  backend_address_pool_id = azurerm_lb_backend_address_pool.lb-internal-backend-001.id
  virtual_network_id      = azurerm_virtual_network.vnet-001.id
  ip_address              = var.palovm.palovm3.trust_ip
  depends_on = [
    azurerm_resource_group.rg-001
  ]
}

resource "azurerm_lb_backend_address_pool_address" "lb-internal-backend-address-002" {

  name                    = "lb_internal_backend_address-palovm4-${var.project}-${var.env}-${var.location}"
  backend_address_pool_id = azurerm_lb_backend_address_pool.lb-internal-backend-001.id
  virtual_network_id      = azurerm_virtual_network.vnet-001.id
  ip_address              = var.palovm.palovm4.trust_ip
  depends_on = [
    azurerm_resource_group.rg-001
  ]
}
